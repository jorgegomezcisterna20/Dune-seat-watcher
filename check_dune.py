...
def get_available_dates() -> list[datetime]:
    ...
    La página arma su contenido con JavaScript, así que en vez de pedirla
    con 'requests' (que solo trae el HTML inicial vacío), usamos Playwright
...
def main() -> None:
    ...
    if not dates:
        print(
            """No se encontraron fechas para Dune: Part Three en esta revision. Puede que la estructura de la pagina haya cambiado, o que la funcion ya no aparezca listada con 'Pre-order'."""
        )
        return
