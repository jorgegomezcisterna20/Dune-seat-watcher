...
1. Revisa la portada de www.cinemark.cl buscando un link a una pelicula
   cuyo nombre/slug contenga "dune" (lectura simple, sin navegador).
2. Revisa la portada de Cinepolis Chile buscando la palabra "DUNE" en el
   listado de peliculas. Esta pagina redirige a un dominio con proteccion
   anti-bots mas fuerte, asi que se usa un navegador invisible (Playwright)
   para leerla, igual que en el monitor de NYC.
...
def check_cinepolis() -> bool:
    ...
    with sync_playwright() as p:
        ...
        page.goto(CINEPOLIS_URL, wait_until="networkidle", timeout=60000)
        html = page.content()
        browser.close()
...
def main() -> None:
    ...
    try:
        ...
        print(f"[Diagnostico Cinepolis] No se pudo revisar: {e}")
        encontrado_cinepolis = False
